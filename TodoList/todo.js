...
const completeTodo = (todoId) => {
	const newTodos = todos.map(todo => (todoId === todo.id ? {... todo, isCompleted:!todo.isCompleted} : todo ));
...
const deleteTodo = (todoId) => {
    const newTodos=todos.filter(todo=>todo.id!==todoId);// newTOdos에 현재 todos를 가져와 filter(조건)를 돌린다 -> todo.id랑 todoId가 같지 않은 것만 가져온다
	todos=newTodos;// newTodos를 todos로 한다
...
const paint = (todo) => {
    ...
    checkBtn.addEventListener('click',(e)=>{
        completeTodo(todo.id);// checkBtn에 이벤트 리스너로 'click'을 감지한다. 실행할 함수는 completeTodo
        //console.log(todo.id);
    });
    ...
    delBtn.addEventListener('click',(e)=>{
        deleteTodo(todo.id);// delBtn에 이벤트 리스너로 'click'을 감지한다. 실행할 함수는 deleteTodo
    });
